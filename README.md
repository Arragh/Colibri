...
2) Circuit Breaker.
3) Rate Limiting.
4) Поддержка протоколов: WebSocket, gRPC.
...
1) "Почему нет поддержки https?" - Потому что https настраивается в nginx или его аналогах. Нет смысла вешать эту нагрузку на API Gateway.